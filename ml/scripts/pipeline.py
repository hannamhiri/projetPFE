...
# Load -> EDA -> Decomp -> Models -> Eval -> Predict -> Registry
# =============================================================
...
from models        import run_h1_modeling, select_best, run_ab_testing
...
def run_pipeline():
    ...
    # ETAPE 7 : Model Registry + A/B Testing
    print('\n>>> ETAPE 7 — Model Registry + A/B Testing')
    winner = run_ab_testing(ts, all_results)
    print(f'  -> Modele en production : {winner}')

    elapsed = time.time() - start
    print(f'\n{"="*60}')
    print(f'  PIPELINE TERMINE en {elapsed:.1f}s')
    print(f'  Meilleur modele   : {best["name"]} (MAPE={best["MAPE"]:.2f}%)')
    print(f'  Modele production : {winner}')
    ...
    return {'ts':ts,'eda':eda_stats,'decomp':decomp_results,
            ...
            'best':best,'df_pred':df_pred,'winner':winner}
